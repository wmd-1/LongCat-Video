...
def generate(args):
    ...
    left_audio_name = Path(left_raw_speech_path).stem if left_raw_speech_path else "silent"
    right_audio_name = Path(right_raw_speech_path).stem if right_raw_speech_path else "silent"
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    video_name = f"{image_name}_{left_audio_name}_{right_audio_name}_{timestamp}"
    ...
    # ---- 分组加载 DiT：每 2 个 rank 一批同时加载，避免所有进程同时读盘导致内存峰值过高 ----
    _load_batch_size = 2
    timer.start("[模型] DiT (分组加载)")
    for _batch_start in range(0, num_processes, _load_batch_size):
        _batch_end = min(_batch_start + _load_batch_size, num_processes)
        if _batch_start <= global_rank < _batch_end:
            timer.log(f"[分组加载] Rank {global_rank} (批次 {global_rank // _load_batch_size + 1}) 开始加载 DiT 模型...")
            if model_type == "avatar-v1.0":
                dit = LongCatVideoAvatarTransformer3DModel.from_pretrained(checkpoint_dir, subfolder="avatar_multi", cp_split_hw=cp_split_hw, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True)
            elif model_type == "avatar-v1.5":
                if use_int8:
            ...
            timer.log(f"[分组加载] Rank {global_rank} DiT 模型加载完成")
        # 每批加载完毕后设 barrier，下一批再开始
        dist.barrier()
    timer.stop()
    ...
    if cp_rank == 0:
        ...
        save_video_ffmpeg(output_tensor, os.path.join(output_dir, video_name), merge_speech_path, fps=save_fps, quality=5)
        timer.stop()
    del output
    torch_gc()
    ...
    for segment_idx in range(1, num_segments):
        ...
        if cp_rank == 0:
            ...
            save_video_ffmpeg(output_tensor, os.path.join(output_dir, f"{video_name}_continue_{segment_idx+1}"), merge_speech_path, fps=save_fps, quality=5)
            timer.stop()
            del output_tensor
